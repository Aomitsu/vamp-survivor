...
use crate::{asset_server, components::{gameplay::{Damage, DamagePlayer, Health, Speed}, physic::CollideWith, render::Sprite, tags::{Player, Projectile}, transform::Transform}};
...
pub fn detect_projectiles_collision(world: &mut World) {
    for (_id, (_projectile, collide_with, damage_player, damage)) in world.query::<(&Projectile, &CollideWith, &DamagePlayer, &Damage)>().iter() {
